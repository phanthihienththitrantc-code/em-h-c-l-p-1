...
import { UserCheck, Users, GraduationCap, ChevronRight, Sparkles, Heart, UserCircle } from 'lucide-react';
...
const LoginView: React.FC<LoginViewProps> = ({ students, classrooms, onSelectStudent, onSelectTeacher, onSelectParent }) => {
  const [view, setView] = useState<'selection' | 'student-list' | 'parent-list'>('selection');
      ...
      <div className="w-full max-w-4xl space-y-12">
        ...
        {view === 'selection' ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in slide-in-from-bottom-8 duration-500">
            ...
            <RoleCard 
              title="Phụ Huynh" 
              description="Ba mẹ theo dõi tiến độ học tập và trang trí ứng dụng cho con." 
          ...
          <div className="space-y-8 animate-in slide-in-from-right-8 duration-500">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {filteredStudents.length > 0 ? filteredStudents.map(student => (
                <button
                  key={student.id}
                  onClick={() => view === 'student-list' ? onSelectStudent(student) : onSelectParent(student)}
